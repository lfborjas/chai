 ...
 * ### .change(obj)
 *
 * Assigns an object and a method/property as a
 * subject for a `change-by` test.
 *
 * @name change
 * @param {Object} object
...
Assertion.prototype.change = function (obj, prop) {
  flag(this, 'target', { obj: obj, prop: prop });
  return this;
};
 ...
 * ### .by(diff)
 *
 * Asserts that a subject is changed by a given value from the
 * execution of a function.
 *
 *     expect(fn).to.change(obj, 'something').by(1);
 *
 * @name change
 * @param {Object} object
 * @param {String} property
...
Assertion.prototype.by = function (diff) {
  var target = flag(this, 'target')
    , subject = target.obj
    , property = target.prop
    , before = util.getValue(property, subject);

  flag(this, 'object')();

  this.assert(
      diff === util.getValue(property, subject) - before
    , 'expected #{this} to change the subject by ' + diff
    , 'expected #{this} to not change the subject by ' + diff
  );
